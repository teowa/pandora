...
public class AppConfigurationReplicaResourceTests : TerraformResourceTestDefinition
{
    public string BasicTestConfig => @"
resource 'azurerm_app_configuration_replicas' 'test' {
...
resource 'azurerm_app_configuration_replicas' 'import' {
  configuration_store_name = 'foo'
  location                 = azurerm_resource_group.test.location
  name                     = 'acctest-${local.random_integer}'
  resource_group_name      = azurerm_resource_group.test.name
